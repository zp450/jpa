...
import com.zp.Jpa.entity.wexin.MyX509TrustManager;
...
public class CommonUtil {
	...
	public static String httpsRequest(String requestUrl, String requestMethod, String outputStr) {
		try {
			...
			// String resultStr = xmlSerializer.read(buffer.toString()).toString();
	 ...
	 * @return
	 *//*
		 * public static Token getToken(String appid, String appsecret) { Token token =
		 * null; String requestUrl = ConfigUtil.TOKEN_URL.replace("APPID",
		 * appid).replace("APPSECRET", appsecret); // 发起GET请求获取凭证 JSONObject jsonObject
		 ...
		 * e) { token = null; // 获取token失败 log.error("获取token失败 errcode:{} errmsg:{}",
		 * jsonObject.getInt("errcode"), jsonObject.getString("errmsg")); } } return
		 * token; } public static String urlEncodeUTF8(String source){ String result =
		 * source; try { result = java.net.URLEncoder.encode(source,"utf-8"); } catch
	...
	private String generateOrderInfoByWeiXinPay(String orderId, float actualPay, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String notify_url = ConstantUtil.WEI_XIN_NOTIFY_URL;// 回调地址
		//// 请求的实际ip地址
		...
		signParams.put("mch_id", ConstantUtil.mch_id);// 微信商户账号
		...
		signParam.put("appid", ConstantUtil.appid);// app_id
		signParam.put("partnerid", ConstantUtil.mch_id);// 微信商户账号
		...
		weiXinVo.append("appid=").append(ConstantUtil.appid).append("&partnerid=").append(ConstantUtil.mch_id)
				.append("&prepayid=").append(prepay_id).append("&package=Sign=WXPay").append("&noncestr=").append(uuid)
				.append("&timestamp=").append(seconds).append("&sign=").append(signAgain);// 拼接参数返回给移动端
