...
//		System.out.println("从请求映射方法上取出权限对象=>"+permission);
	 ...
	 * @return 权限对象
	 */
	public static String method2PermissionValue(Method method) {
		// TODO Auto-generated method stub
		String permissionValue = null;//请求映射方法上的权限字符串
		RequestMapping anno = method.getAnnotation(RequestMapping.class);//从控制器映射方法上取出@RequestMapper注解对象
		//@RequestMapper注解有没有name备注是作为一个权限的标志
		if( !"".equals(anno.name()) ){//@RequestMapper注解写了name属性才做权限收集：所以@RequestMapper注解有没有name备注是作为一个权限的标志
//			Class<?> ok=method.getReturnType();
//			RequestMapping namespaceMapping = method.getReturnType().getAnnotation(RequestMapping.class);//带有@RequestMapper注解的方法所在控制器类上的RequestMapping注解对象
//			String namespace =  namespaceMapping.value()[0];//得到RequestMapping注解的value值,即命名空间,即模块
			Class clazz =method.getDeclaringClass();
			RequestMapping annotation=(RequestMapping) clazz.getAnnotation(RequestMapping.class);
			String name=annotation.value()[0].replace("/", "");
			
			permissionValue = (name+":"+anno.value()[0]).replace("/", "");//得到权限 ,例如：user:delete 用户模块的删除权限
//			permissionValue=anno.value()[0];
//			System.out.println("得到权限 ,例如：user:delete 用户模块的删除权限=>"+permissionValue +"权限说明:"+anno.name());
			//构造权限对象,三个参数:权限,模块说明,权限说明
//			permission = new SysPermission(permissionValue,namespaceMapping.name(), anno.name());//把权限控制注解@Permission解析为权限控制对象SysPermission,方便插入数据库权限表
		}
